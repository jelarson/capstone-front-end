...
export default function BrailleQuizTwo(props) {
  const [currentQuestion, setCurrentQuestion] = useState({})
  const [selectedAnswer, setSelectedAnswer] = useState('')
  const [selectedAnswerUrl, setSelectedAnswerUrl] = useState('')
  const [path, setPath] = useState(`/braille-mastery/q2/${slug}`)
  ...
  useEffect(() => {
    if (selectedAnswer === currentQuestion.answerName) {
      setPath(`/correct/braille/q2/${slug}`)
    } else {
      setPath(`/wrong/braille/q2/${slug}`)
    }
  }, [selectedAnswer])
